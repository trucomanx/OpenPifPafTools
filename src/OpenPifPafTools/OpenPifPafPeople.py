...
def get_pil_images_with_people(filepath, factor=1.4,face=False):
    '''
    ...
    for annot in annotation:
        if face==True:
            tupla=oppd.get_face_bounding_rectangle(annot.data, factor=factor);
        else:
            tupla=oppd.get_body_bounding_rectangle(annot.data, factor=factor);
